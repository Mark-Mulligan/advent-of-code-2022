...
const lines = testResult.split(/\r?\n/);
...
const doOperation = (oldValue: any, operation: string) => {
  ...
  if (op === '+') return BigInt(num1) + BigInt(num2);
  else if (op === '*') return BigInt(num1) * BigInt(num2);
  return BigInt(num1) + BigInt(num2);
};

const doTest = (worryLevel: bigint, test: string) => {
  let divider = BigInt(test.split(' ')[2]);
  return worryLevel % divider === BigInt(0);
};
...
const round = () => {
  Object.keys(monkeyCollection).forEach((monkey) => {
    ...
    while (startingItems.length > 0) {
      let currentItem = BigInt(startingItems.shift());
      monkeyCollection[monkey].inspectionCount += 1;
      let worryLevel = doOperation(currentItem, operation);
